...
Bu bölümde, ağ uygulamalarının (**network applications**) kavramsal (**conceptual**) ve uygulama (implementation aspects) yönlerini inceledik. 
Birçok İnternet uygulaması (**Internet applications**) tarafından benimsenen yaygın istemci-sunucu mimarisi (**ubiquitous client-server architecture**) hakkında bilgi edindik ve bunun HTTP, SMTP ve DNS protokollerinde (**HTTP, SMTP, and DNS protocols**) kullanımını gördük. 
Bu önemli uygulama katmanı protokollerini (**application-level protocols**) ve bunlara karşılık gelen ilişkili uygulamaları (**corresponding associated applications**) (Web, dosya aktarımı - **file transfer**, e-posta - **e-mail** ve DNS - **DNS**) detaylı olarak inceledik. 
P2P mimarisini (**P2P architecture**) öğrendik ve bunu istemci-sunucu mimarisiyle (**client-server architecture**) karşılaştırdık (**contrasted**). 
Ayrıca video akışı (**streaming video**) ve modern video dağıtım sistemlerinin (**modern video distribution systems**) CDN'lerden (**CDNs**) nasıl yararlandığını da öğrendik. Ağ uygulamaları oluşturmak (**build network applications**) için soket API'sinin (**socket API**) nasıl kullanılabileceğini inceledik. 
Bağlantı yönelimli (TCP) (**connection-oriented (TCP)**) ve bağlantısız (UDP) (**connectionless (UDP)**) uçtan uca taşıma hizmetleri (**end-to-end transport services**) için soketlerin (**socket**) kullanımını adım adım gözden geçirdik. 
Katmanlı ağ mimarisindeki (**layered network architecture**) yolculuğumuzun ilk adımı şimdi tamamlandı!

Protokolün (**protocol**) oldukça belirsiz, sade bir tanımını (**vague, bare-bones definition**) vermiştik: "İki veya daha fazla iletişim kuran varlık (**communicating entities**) arasında değiş tokuş edilen mesajların biçimi (**format**) ve sırası (**order of messages exchanged**), ayrıca bir mesajın iletimi ve/veya alınması veya diğer olaylar üzerine yapılan eylemler (**actions taken on the transmission and/or receipt of a message or other event**)." Bu bölümdeki materyal ve özellikle HTTP, SMTP ve DNS protokolleri üzerine detaylı çalışmamız, bu tanıma önemli ölçüde içerik (**considerable substance**) katmıştır. Protokoller, ağ oluşturmada (**networking**) temel bir kavramdır (**key concept**); uygulama protokolleri çalışmamız, bize protokollerin ne hakkında olduğuna dair daha sezgisel bir his (**intuitive feel**) geliştirme fırsatı vermiştir.

TCP ve UDP'nin (**TCP and UDP**) kendilerini çağıran uygulamalara (**applications that invoke them**) sunduğu hizmet modellerini (**service models**) açıkladık. TCP ve UDP üzerinde çalışan basit uygulamalar (**simple applications that run over TCP and UDP**) geliştirdiğimizde bu hizmet modellerine daha yakından baktık. Ancak, TCP ve UDP'nin bu hizmet modellerini nasıl sağladığı hakkında çok az şey söyledik. Örneğin, TCP'nin güvenilir bir veri hizmeti (**reliable data service**) sağladığını biliyoruz, ancak bunu nasıl yaptığını henüz söylemedik. Sonraki bölümde, taşıma protokollerinin (**transport protocols**) sadece ne olduğunu değil, aynı zamanda **nasıl ve neden** (**how and why**) yaptıklarını dikkatlice inceleyeceğiz.

İnternet uygulama yapısı (**Internet application structure**) ve uygulama katmanı protokolleri (**application-level protocols**) hakkındaki bilgiyle donanmış olarak, şimdi protokol yığınında (**protocol stack**) daha aşağı inmeye ve Bölüm 3'te taşıma katmanını (**transport layer**) incelemeye hazırız.
