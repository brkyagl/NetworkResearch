...
#### Kötü Niyetli Kişiler Paketleri Dinleyebilir

Günümüzde birçok kullanıcı internete WiFi bağlantılı dizüstü bilgisayarlar veya mobil internet bağlantısına sahip el cihazları gibi kablosuz cihazlar aracılığıyla erişmektedir. Her yerde internet erişiminin son derece kullanışlı olması ve mobil kullanıcılar için harika yeni uygulamalar sunmasının yanı sıra, önemli bir güvenlik açığı da yaratır: kablosuz vericinin yakınında pasif bir alıcı konumlandırarak, bu alıcı iletilen her paketin bir kopyasını alabilir! Bu paketler, şifreler, sosyal güvenlik numaraları, ticari sırlar ve özel kişisel mesajlar dahil olmak üzere her türlü hassas bilgiyi içerebilir. Geçen her paketin bir kopyasını kaydeden pasif bir alıcıya **paket dinleyici (packet sniffer)** denir.

Dinleyiciler kablolu ortamlarda da kullanılabilir. Birçok Ethernet LAN'ında olduğu gibi kablolu yayın ortamlarında, bir paket dinleyici LAN üzerinden gönderilen yayın paketlerinin kopyalarını alabilir. Kablolu erişim teknolojileri de paketleri yayınlar ve bu nedenle dinlemeye karşı savunmasızdır. 
Dahası, bir kurumun erişim yönlendiricisine veya internete erişim bağlantısına erişim sağlayan kötü niyetli bir kişi, kuruluşa giden/gelen her paketin bir kopyasını oluşturan bir dinleyici yerleştirebilir. Dinlenen paketler daha sonra hassas bilgiler için çevrimdışı olarak analiz edilebilir. 
Paket dinleme yazılımı çeşitli web sitelerinde ücretsiz olarak ve ticari ürünler olarak mevcuttur. Ağ dersi veren profesörlerin, paket dinleme ve uygulama katmanı veri yeniden yapılandırma programı yazmayı içeren laboratuvar ödevleri verdikleri bilinmektedir. 

Paket dinleyiciler pasif oldukları için—yani kanala paket enjekte etmezler—tespit edilmeleri zordur. 
Bu nedenle, kablosuz bir kanala paket gönderdiğimizde, kötü niyetli birinin paketlerimizin kopyalarını kaydediyor olabileceği olasılığını kabul etmeliyiz. 
Tahmin edebileceğiniz gibi, paket dinlemeye karşı en iyi savunmalardan bazıları şifrelemeyi içerir.

#### Kötü Niyetli Kişiler Güvendiğiniz Biri Gibi Davranabilir

Şaşırtıcı derecede kolay bir şekilde, rastgele bir kaynak adresi, paket içeriği ve hedef adresiyle bir paket oluşturup bu el yapımı paketi internete gönderebilirsiniz. İnternet de bu paketi hedefine doğru itaatkar bir şekilde iletecektir. Böyle bir paketi alan şüphelenmeyen bir alıcıyı (örneğin bir internet yönlendiricisi) hayal edin. Bu alıcı, (yanlış olan) kaynak adresini doğru kabul eder ve paketin içeriğine gömülü bir komutu (örneğin yönlendirme tablosunu değiştirme) gerçekleştirir. İnternete yanlış bir kaynak adresiyle paket gönderebilme yeteneği **IP spoofing (IP sahteciliği)** olarak bilinir ve bir kullanıcının başka bir kullanıcı kılığına girebileceği birçok yoldan sadece biridir.

Bu sorunu çözmek için **end-point authentication (uç nokta kimlik doğrulaması)**'na ihtiyacımız olacak. Bu, bir mesajın düşündüğümüz yerden gelip gelmediğini kesin olarak belirlememizi sağlayacak bir mekanizmadır. 

Bu bölümü kapatırken, internetin en başından beri neden bu kadar güvensiz bir yer haline geldiğini düşünmekte fayda var. Cevap aslında, internetin başlangıçta "şeffaf bir ağa bağlı, birbirine güvenen kullanıcılar grubu" [Blumenthal 2001] modeline dayanarak bu şekilde tasarlanmış olmasıdır—bu modelde (tanım gereği) güvenliğe ihtiyaç yoktur. Orijinal internet mimarisinin birçok yönü bu karşılıklı güven kavramını derinlemesine yansıtmaktadır. Örneğin, bir kullanıcının herhangi bir başka kullanıcıya paket gönderebilmesi, talep edilen/izin verilen bir yetenek olmaktan ziyade varsayılandır ve kullanıcı kimliği varsayılan olarak doğrulanmak yerine beyan edilen şekliyle kabul edilir.

Ancak bugünün interneti kesinlikle "birbirine güvenen kullanıcılar" içermiyor. Yine de, günümüz kullanıcıları birbirlerine güvenmek zorunda olmadıkları durumlarda bile iletişim kurmaya ihtiyaç duyuyorlar, anonim olarak iletişim kurmak isteyebilirler, üçüncü taraflar aracılığıyla dolaylı olarak iletişim kurabilirler ve iletişim kurdukları donanıma, yazılıma ve hatta havaya bile güvenmeyebilirler. Bu yazıları okurken önümüzde birçok güvenlik sorunu var: dinlemeye, uç nokta sahteciliğine, **man-in-the-middle attacks (ortadaki adam saldırıları)**'na, DDoS saldırılarına, malware'e ve daha fazlasına karşı savunmalar aramalıyız. Karşılıklı güvenen kullanıcılar arasındaki iletişimin kuraldan ziyade istisna olduğunu unutmamalıyız. Modern bilgisayar ağları dünyasına hoş geldiniz!

#### Bilgisayar Ağı ve İnternet'in Tarihi

Bilgisayar ağı ve internet teknolojisine genel bir bakış sunduk. Artık ailenizi ve arkadaşlarınızı etkileyecek kadar bilgiye sahip olmalısınız! 
Ancak, bir sonraki kokteyl partisinde gerçekten büyük bir etki yaratmak istiyorsanız, konuşmalarınıza internetin büyüleyici tarihi hakkında küçük bilgiler serpiştirmelisiniz [Segaller 1998].

#### Paket Anahtarlamanın Gelişimi: 1961–1972

Bilgisayar ağları ve bugünkü internetin kökleri, 1960'ların başlarına, telefon ağının dünyanın baskın iletişim ağı olduğu zamana kadar uzanır. 
Telefon ağı, bir göndericiden bir alıcıya bilgi iletmek için **circuit switching (devre anahtarlama)** kullanır. 
Bu, sesin gönderici ve alıcı arasında sabit bir hızda iletilmesi göz önüne alındığında uygun bir seçimdi. 
1960'ların başlarında bilgisayarların artan önemi ve zaman paylaşımlı bilgisayarların ortaya çıkmasıyla, coğrafi olarak dağılmış kullanıcılar arasında paylaşılabilecek şekilde bilgisayarların birbirine nasıl bağlanabileceğini düşünmek belki de doğaldı. Bu tür kullanıcılar tarafından oluşturulan **traffic (trafik)** muhtemelen **bursty (ani ve yoğun)** olacaktı—uzak bir bilgisayara bir komut gönderme gibi etkinlik aralıkları, ardından bir yanıt beklerken veya alınan yanıtı düşünürken geçen hareketsizlik dönemleri.

Dünya çapında, her biri diğerinin çalışmalarından habersiz [Leiner 1998] üç araştırma grubu, devre anahtarlamaya verimli ve **robust (güçlü)** bir **alternative (alternatif)** olarak **packet switching (paket anahtarlama)**'i icat etmeye başladı. Paket anahtarlama teknikleri üzerine ilk yayınlanan çalışma, o zamanlar MIT'de yüksek lisans öğrencisi olan Leonard Kleinrock'a aitti [Kleinrock 1961; Kleinrock 1964]. Kleinrock, **queuing theory (kuyruk teorisi)**'ni kullanarak, çalışmasında paket anahtarlama yaklaşımının ani ve yoğun trafik kaynakları için etkinliğini zarif bir şekilde gösterdi. 1964'te Rand Enstitüsü'nden Paul Baran [Baran 1964], askeri ağlar üzerinden güvenli ses iletişimi için paket anahtarlamanın kullanımını araştırmaya başlamıştı ve İngiltere'deki National Physical Laboratory'de Donald Davies ve Roger Scantlebury de paket anahtarlama üzerine fikirlerini geliştiriyorlardı.

MIT, Rand ve NPL'deki çalışmalar, bugünkü internetin temellerini attı. Ancak internet aynı zamanda 1960'lara kadar uzanan bir "hadi inşa edelim ve gösterelim" tutumunun da uzun bir geçmişine sahiptir. J. C. R. Licklider [DEC 1990] ve Lawrence Roberts, her ikisi de MIT'de Kleinrock'un meslektaşlarıydı ve daha sonra Amerika Birleşik Devletleri'ndeki **Advanced Research Projects Agency (ARPA)**'nın bilgisayar bilimi programını yönetmeye başladılar. 
Roberts, ilk paket anahtarlamalı bilgisayar ağı ve bugünün halka açık internetinin doğrudan atası olan **ARPAnet** için genel bir plan yayınladı [Roberts 1967]. 1969'daki İşçi Bayramı'nda, ilk **packet switch (paket anahtarı)** Kleinrock'un gözetiminde UCLA'ya kuruldu ve kısa bir süre sonra Stanford Research Institute (SRI), UC Santa Barbara ve Utah Üniversitesi'ne üç ek paket anahtarı kuruldu. İnternetin bu ilk öncüsü, 1969'un sonunda dört düğüm büyüklüğündeydi. 
Kleinrock, UCLA'dan SRI'ye yapılan ilk **remote login (uzaktan oturum açma)** denemesinin sistemi çökerttiğini hatırlıyor [Kleinrock 2004].

1972'ye gelindiğinde, ARPAnet yaklaşık 15 düğüme ulaşmıştı ve Robert Kahn tarafından ilk kez halka açık bir şekilde tanıtıldı. 
ARPAnet **end systems (uç sistemleri)** arasındaki ilk **host-to-host protocol (ana bilgisayardan ana bilgisayara protokol)**, **network-control protocol (NCP)** tamamlandı [RFC 001]. Bir **end-to-end protocol (uçtan uca protokol)** mevcut olduğunda, artık uygulamalar yazılabiliyordu. 
Ray Tomlinson, ilk **e-mail program (e-posta programı)**'nı 1972'de yazdı.

#### Tescilli Ağlar ve Ağlararası Bağlantı: 1972–1980

İlk ARPAnet tek ve kapalı bir ağdı. Bir ARPAnet ana bilgisayarıyla iletişim kurmak için, gerçekten başka bir ARPAnet IMP'sine bağlı olmak gerekiyordu. 
1970'lerin başlarından ortalarına kadar, ARPAnet'in yanı sıra başka bağımsız paket anahtarlamalı ağlar da ortaya çıktı: Hawaii adalarındaki üniversiteleri birbirine bağlayan bir mikrodalga ağı olan ALOHANet [Abramson 1970], DARPA'nın paket uydu [RFC 829] ve paket radyo ağları [Kahn 1978]; ARPAnet teknolojisine dayanan bir BBN ticari paket anahtarlamalı ağı olan Telenet; Louis Pouzin tarafından öncülük edilen bir Fransız paket anahtarlamalı ağı olan Cyclades [Think 2012]; 1960'ların sonlarında ve 1970'lerin başlarında Tymnet ve GE Information Services ağı gibi zaman paylaşımlı ağlar [Schwartz 1977]; IBM'in ARPAnet çalışmasına paralel olarak geliştirilen SNA'sı (1969–1974) [Schwartz 1977].

Ağ sayısı giderek artıyordu. Geriye dönüp baktığımızda, ağları birbirine bağlamak için kapsamlı bir mimari geliştirmenin zamanının geldiğini görebiliyoruz. 
Ağları birbirine bağlama konusunda (Savunma İleri Araştırma Projeleri Ajansı (DARPA) sponsorluğunda), aslında bir ağlar ağı yaratma konusunda öncü çalışmalar Vinton Cerf ve Robert Kahn tarafından yapıldı [Cerf 1974]; bu çalışmayı tanımlamak için **internetting (ağlararası çalışma)** terimi ortaya atıldı.

Bu mimari prensipler TCP'de somutlaştırıldı. Ancak TCP'nin ilk sürümleri bugünkü TCP'den oldukça farklıydı. 
TCP'nin ilk sürümleri, uç sistem yeniden iletimi yoluyla verinin güvenilir ve sıralı teslimatını (bugünkü TCP'nin hala bir parçası) yönlendirme işlevleriyle (bugün IP tarafından gerçekleştirilen) birleştiriyordu. TCP ile yapılan erken deneyler ve paketlenmiş ses gibi uygulamalar için güvenilir olmayan, akış kontrolsüz, uçtan uca bir taşıma hizmetinin öneminin anlaşılması, IP'nin TCP'den ayrılmasına ve UDP protokolünün geliştirilmesine yol açtı. Bugün gördüğümüz üç temel internet protokolü—TCP, UDP ve IP—1970'lerin sonuna doğru kavramsal olarak yerini almıştı.
